...
// Legacy Google Apps Script API client
// This file is kept for reference and rollback purposes
// New implementations should use services/supabase.ts
...
const isApiConfigured = () => {
  return false; // Disabled - migrated to Supabase
...
const apiRequest = async (payload: any, retries = 2): Promise<ApiResponse> => {
    ...
    try {
        ...
        throw new Error('Google Apps Script API is disabled. Use Supabase instead.');
    } catch (error: any) {
        if (retries > 0) {
