...
import { deleteEstimate, markJobPaid, createWorkOrderSheet, syncUp, startJob, completeJob } from '../services/supabase';
...
export const useEstimates = () => {
  ...
  const handleDeleteEstimate = async (id: string, e?: React.MouseEvent) => {
    ...
    if (confirm("Are you sure you want to delete this job?")) {
      ...
      if (session) {
          try {
              await deleteEstimate(id, session.username);
  ...
  const handleMarkPaid = async (id: string) => {
      ...
      if (estimate) {
         ...
         const result = await markJobPaid(id, session?.username || '');
  ...
  const handleBackgroundWorkOrderGeneration = async (record: EstimateRecord, currentWarehouse: any) => {
      ...
      try {
          ...
          // In Supabase, this would be an Edge Function or just store metadata
          const woUrl = await createWorkOrderSheet(record, session.username);
          ...
          await syncUp(updatedState, session.username);
