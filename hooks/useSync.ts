...
import { syncUp, syncDown } from '../services/supabase';
...
export const useSync = () => {
  ...
  useEffect(() => {
    ...
    const initializeApp = async () => {
      ...
      try {
          ...
          // Use username as userId for legacy compatibility, or use session.token to extract user ID
          const userId = session.username; 
          const cloudData = await syncDown(userId);
  ...
  useEffect(() => {
    ...
    syncTimerRef.current = setTimeout(async () => {
      ...
      const success = await syncUp(appData, session.username);
  ...
  const forceRefresh = async () => {
      ...
      dispatch({ type: 'SET_SYNC_STATUS', payload: 'syncing' });
      try {
          const cloudData = await syncDown(session.username);
          if (cloudData) {
