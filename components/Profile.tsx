...
export const Profile: React.FC<ProfileProps> = ({ state, onUpdateProfile, onManualSync, syncStatus, username, userId }) => {
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
        ...
        reader.onloadend = async () => {
            ...
            if (userId || username) {
                const url = await uploadImage(base64, userId || username!, "company_logo.jpg", 'logos');
                if (url) {
